...
// Return a Tensor view of `shape` backed by a persistent, module-owned buffer,
// growing the buffer if needed. Replaces per-forward Tensor allocation for
// scratch outputs: the owning Tensor constructor zero-fills its whole buffer
// on every call, which for the per-layer intermediates adds a full write pass
// over megabytes of scratch per decoded token plus heap churn. Every user of
// this helper fully overwrites the viewed range before reading it (matmul
// outputs, norm outputs, conv outputs), so skipping the zero-fill is
// bit-identical. Growth zero-fills only the newly added region.
inline Tensor scratch_view(std::vector<float>& buf, const std::vector<int>& shape) {
    long long n = 1;
    for (int d : shape) n *= d;
    if ((long long)buf.size() < n) buf.resize(n, 0.0f);
    return Tensor(shape, buf.data());
}
...
public:
    ...
    void forward(const Tensor& input, Tensor& output, Context& ctx) override {
        ...
        Tensor gate_out = scratch_view(s_gate, {seq_len, intermediate_size});
        Tensor up_out = scratch_view(s_up, {seq_len, intermediate_size});
...
private:
    ...
    // Persistent scratch backing for gate_out/up_out (see scratch_view).
    std::vector<float> s_gate, s_up;
};
...
public:
    ...
    void forward(const Tensor& input, Tensor& output, Context& ctx) override {
        ...
        // --- Attention block ---
        Tensor norm1_out = scratch_view(s_norm1, input.shape);
        norm1->forward(output, norm1_out, ctx);

        Tensor attn_out = scratch_view(s_attn, input.shape);
        attn->forward(norm1_out, attn_out, ctx);

        // --- MLP block ---
        Tensor norm2_out = scratch_view(s_norm2, input.shape);
        if (fusion_enabled) {
        ...
        Tensor mlp_out = scratch_view(s_mlp, input.shape);
...
private:
    ...
    bool fusion_enabled = false;
    // Persistent scratch backing for the per-forward intermediates (see scratch_view).
    std::vector<float> s_norm1, s_attn, s_norm2, s_mlp;
};
